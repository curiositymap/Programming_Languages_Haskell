...
type Prog = [Stmt] -- synonym for something you already have -- the only difference is clarity/simplicity

{- For example, you want `Counter` for a specific purpose for tracking int...}
type Counter = Int -}

-- Question 2. Encode the AST of the example program above as a Haskell value.
example :: Prog
example = [Store A (LitInt 7)
           , Store B (LitInt 9)
           , Store R (LitInt 0)
           , Do
            [IfElse (Leq (Load A) (LitInt 0))
                    [Break]
                    [Store R (Add (Load R) (Load B))
                   , Store A (Add (Load A) (LitInt (-1)))
                    ]
            ]
          ]
...
-- First attempt: while a b = IfElse (Leq (Load A) (Load B)) [Store R (Leq (Load A) (Load B))] [Break]
while c b = Do [IFElse c b [Break]]
...
-- First attempt: sumFromTo x y = [IfElse (Leq (LitInt x) (LitInt y)) (sumFromTo (x+1) y) [Break]]
sumFromTo x y =
  [ Store R (LitInt 0)
  , Store A (LitInt x)
  , while (Leq (Load A)(LitInt y))
    [ Store R (Add (Load R)(Load A))
    , Store A (Add (Load A)(LitInt 1))
    ]
  ]
